...
This file provides the foundational context and operational mandates for AI agents working on the StatVision project.

## Database Credentials (from env vars)
- `STATVISION_DB` → database name
- `STATVISION_DB_USER` → username
- `STATVISION_DB_PASS` → password
...
7. **QA Readiness:** After each development cycle or feature implementation, the developer MUST create a detailed "QA Task List" (typically as a new section in `jobLog.md`) specifying exactly what needs to be tested, expected behaviors, and any edge cases to verify the changes.

## 🚧 Environment Limitations (2026-07-06)
1. **Test env not fully published:** The `test` branch deploys but the `-test` Cloud Run services are not fully configured for external access. The agent has access to **production only** (`master` branch deploys to `-prod`).
2. **Direct-to-master workflow:** Since test environment isn't usable, the workflow is:
   - Commit to a feature branch
   - Merge directly to `master`
   - CI/CD deploys to production (`-prod`)
   - Test on production
