...
## 🔄 Stabilization Sprint: Review-Led Hardening (COMPLETED)
- [x] **Virtual Chunking Migration:** Pivot from physical FFmpeg slicing to offset-based logical segments. (`VideoChunkerService.generateVirtualChunks()` — default mode, no FFmpeg needed)
- [x] **Robust Job State Machine:** Implement `currentPhase` and `heartbeatAt` tracking in the DB. (`VideoAnalysisJob.currentPhase`, `processingHeartbeatAt` columns)
- [x] **API Watchdog:** Automated detection and recovery for stale worker jobs. (`JobWatchdogService` via Cloud Scheduler + `janitorService` on worker)
...
- [x] **Virtual Coach Report (V1):** AI-powered insights (Strengths/Weaknesses/Drills) based on game data.
